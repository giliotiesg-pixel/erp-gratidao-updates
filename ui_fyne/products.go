...
func buildProducts(s *Store,w fyne.Window)fyne.CanvasObject{
 ...
 openForm:=func(existing *productFull){
  ...
  promoBox:=container.NewVBox(promo,widget.NewForm(widget.NewFormItem("Desconto da promoção (%)",promoDiscount)),promoPrice,margin,widget.NewForm(widget.NewFormItem("Forma de venda",saleMode)))
