...
func buildProducts(s *Store,w fyne.Window)fyne.CanvasObject{
 ...
 openForm:=func(existing *productFull){
  ...
  promoBox:=container.NewVBox(promo,widget.NewForm(widget.NewFormItem("Desconto da promoção (%)",promoDiscount)),promoPrice,margin)
 ...
 newBtn:=widget.NewButton("Cadastrar Produto",func(){openForm(nil)});newBtn.Importance=widget.HighImportance
 ...
 header:=container.NewVBox(widget.NewLabelWithStyle("Produtos cadastrados",fyne.TextAlignLeading,fyne.TextStyle{Bold:true}),container.NewBorder(nil,nil,search,container.NewHBox(filter,clearSearch,newBtn,editBtn)))
