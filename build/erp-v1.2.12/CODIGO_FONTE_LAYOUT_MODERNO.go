...
func roundControl(hw uintptr, w, h, radius int) {
	...
	if rgn != 0 {
		// Após SetWindowRgn com sucesso, o Windows passa a ser dono da região.
...
func add(class, text string, style uint32, x, y, w, h, id int) uintptr {
	hw, _, _ := pCreateWindowExW.Call(0, uintptr(unsafe.Pointer(ws(class))), uintptr(unsafe.Pointer(ws(text))), uintptr(func() uint32 {
		if strings.EqualFold(class, "BUTTON") {
			return style | WS_CHILD | WS_VISIBLE | BS_OWNERDRAW
		}
		return style | WS_CHILD | WS_VISIBLE
	}()), uintptr(x), uintptr(y), uintptr(w), uintptr(h), mainWnd, uintptr(id), 0, 0)
	if font != 0 {
		pSendMessageW.Call(hw, WM_SETFONT, font, 1)
	}
	// Acabamento moderno sem alterar IDs, eventos ou regras dos módulos.
	switch strings.ToUpper(class) {
	case "BUTTON":
		roundControl(hw, w, h, 16)
	case "EDIT", "COMBOBOX":
		roundControl(hw, w, h, 12)
	case "LISTBOX":
...
func showBoot() {
	currentModule = "boot"
	clearContent()
	header("ERP GRATIDÃO", "Sistema de Gestão Comercial • Armazém Gratidão")
...
func showInicio() {
	...
	clearContent()
	header("ERP GRATIDÃO", "Sistema de Gestão Comercial • Painel principal")
	...
	section("Armazém Gratidão • Sempre com você", 250, 390, 650)
	...
	add("STATIC", "Controle suas vendas • Gerencie seu estoque • Cadastre seus clientes • Relatórios completos", 0, 250, 470, 1000, 32, 0)
}
...
const officialManifestURL = "https://raw.githubusercontent.com/giliotiesg-pixel/erp-gratidao-updates/main/erp/versao.json"
...
func versionGreater(a, b string) bool {
	...
	for i := 0; i < 3; i++ {
		ai, e1 := strconv.Atoi(pa[i])
		bi, e2 := strconv.Atoi(pb[i])
		if e1 != nil || e2 != nil {
			return false
		}
		if ai > bi {
...
func checkUpdatesAsync(silent bool) {
	go func() {
		...
		if r.StatusCode != 200 {
			if !silent {
				setUpdateError(fmt.Errorf("manifesto HTTP %d", r.StatusCode))
			}
			return
		}
		...
		if !strings.HasPrefix(strings.ToLower(m.PackageURL), "https://") || !strings.HasPrefix(strings.ToLower(m.UpdaterURL), "https://") || !validSHA(m.SHA256) || !validSHA(m.UpdaterSHA256) {
			if !silent {
				setUpdateError(fmt.Errorf("publicação do GitHub ainda não está completa"))
			}
			return
		}
		if e = checkURL(&c, m.PackageURL); e != nil {
			if !silent {
				setUpdateError(fmt.Errorf("pacote indisponível: %v", e))
			}
			return
		}
		if e = checkURL(&c, m.UpdaterURL); e != nil {
			if !silent {
				setUpdateError(fmt.Errorf("atualizador indisponível: %v", e))
			}
			return
		}
		updateMu.Lock()
...
func showUpdater() {
	...
	clearContent()
	header("Atualizações", "Atualizador nativo do ERP Gratidão — somente libera versões completas no GitHub.")
	section("Atualizador de versões", 250, 135, 900)
	add("STATIC", "Canal oficial: GitHub / ERP Gratidão", 0, 255, 190, 700, 28, 0)
	add("BUTTON", "Verificar agora", 0, 255, 245, 150, 42, 3203)
	add("STATIC", "A atualização só aparece quando manifesto, pacote ZIP, atualizador e SHA-256 estiverem publicados e acessíveis.", 0, 255, 315, 940, 70, 0)
}
func saveUpdateURL() {
	msg("O endereço de atualização agora é oficial e fixo. Não é necessário configurar URL.")
}
func installUpdateAsync(m *UpdateManifest) {
	...
	go func() {
		...
		tmp := filepath.Join(upd, "ATUALIZADOR_GRATIDAO.exe.download")
		final := filepath.Join(upd, "ATUALIZADOR_GRATIDAO.exe")
		os.Remove(tmp)
		...
		defer r.Body.Close()
		if r.StatusCode != 200 {
			setUpdateError(fmt.Errorf("download do atualizador HTTP %d", r.StatusCode))
			return
		}
		f, e := os.Create(tmp)
		if e != nil {
		...
		h := sha256.New()
		_, e = io.Copy(io.MultiWriter(f, h), io.LimitReader(r.Body, 128*1024*1024))
		f.Close()
		if e != nil {
			setUpdateError(e)
			return
		}
		if !strings.EqualFold(hex.EncodeToString(h.Sum(nil)), strings.TrimSpace(m.UpdaterSHA256)) {
			os.Remove(tmp)
			setUpdateError(fmt.Errorf("SHA-256 do atualizador não confere"))
			return
		}
		os.Remove(final)
...
func launchPreparedUpdate() {
	...
	updateMu.Unlock()
	if len(parts) != 5 {
		msgErr("Atualização preparada inválida.")
		return
	}
	execSQL("PRAGMA wal_checkpoint(FULL)")
	cmd := exec.Command(parts[0], parts[1], parts[2], parts[3], parts[4])
	cmd.Dir = root
	if e := cmd.Start(); e != nil {
		msgErr("Não foi possível iniciar o atualizador: " + e.Error())
		return
	}
	if db != 0 {
		pSqlClose.Call(db)
...
func lookupProductOnline() {
	...
	if len(barcode) < 8 || len(barcode) > 14 || strings.Trim(barcode, "0123456789") != "" {
		msgErr("Informe um código de barras numérico de 8 a 14 dígitos.")
		return
	}
	...
	go func(code string) {
		...
		req, err := http.NewRequest("GET", "https://world.openfoodfacts.org/api/v2/product/"+code+".json?fields=product_name,brands,categories", nil)
		if err != nil { return }
		req.Header.Set("User-Agent", "ERP-Gratidao/1.2.12 (consulta de produtos)")
		...
		if e == nil {
			...
			if resp.StatusCode != http.StatusOK {
				failure = fmt.Sprintf("Consulta online indisponível (HTTP %d). Tente novamente.", resp.StatusCode)
			} else {
				var op onlineProduct
				if json.NewDecoder(io.LimitReader(resp.Body, 2<<20)).Decode(&op) == nil && op.Status == 1 {
		...
		lookupData = data
		if e != nil {
			lookupErr = "Não foi possível acessar a internet. Confira a conexão e tente novamente."
		} else if failure != "" {
			lookupErr = failure
...
func loadSales(today bool) {
	listReset(salesList)
	salesIDs = nil
	q := "SELECT id,sale_number,datetime(created_at,'localtime'),customer_name,payment_method,printf('%.2f',total),status FROM sales"
	if today {
		q += " WHERE date(created_at,'localtime')=date('now','localtime')"
	}
...
func drawModernButton(dis *DRAWITEMSTRUCT) {
	...
	if text == "☰" || text == "⚙" {
		pSelectObject.Call(dis.HDC, fontBig)
	}
...
func wndProc(hwnd uintptr, m uint32, w, l uintptr) uintptr {
	...
	case WM_APP_LOOKUPDONE:
		...
		d := lookupData
		lookupMu.Unlock()
		if prodBarcodeReg != 0 && d != nil && strings.TrimSpace(getText(prodBarcodeReg)) == d["barcode"] {
	...
	case WM_APP_LOOKUPFAIL:
		lookupMu.Lock()
		e := lookupErr
		code := lookupData["barcode"]
		lookupMu.Unlock()
		if e != "" && prodBarcodeReg != 0 && strings.TrimSpace(getText(prodBarcodeReg)) == code {
			msg(e)
		}
		return 0
	case WM_APP_DBFAIL:
...
func buildNavigation() {
	...
	brushTop, _, _ = pCreateSolidBrush.Call(0x00BB4D0B)           // RGB aproximado #0b3158 em COLORREF BGR
	brushRed, _, _ = pCreateSolidBrush.Call(0x002019D7)           // #c81d25
	brushButton, _, _ = pCreateSolidBrush.Call(0x00B85B13)        // azul moderno #135BB8
	brushButtonPressed, _, _ = pCreateSolidBrush.Call(0x008C430D) // azul pressionado
	...
	brand, _, _ := pCreateWindowExW.Call(0, uintptr(unsafe.Pointer(ws("STATIC"))), uintptr(unsafe.Pointer(ws("ERP GRATIDÃO"))), WS_CHILD|WS_VISIBLE, 56, 11, 250, 32, mainWnd, 0, 0, 0)
	...
	ver, _, _ := pCreateWindowExW.Call(0, uintptr(unsafe.Pointer(ws("STATIC"))), uintptr(unsafe.Pointer(ws("v1.2.12"))), WS_CHILD|WS_VISIBLE, 820, 18, 48, 20, mainWnd, 0, 0, 0)
...
func main() {
	...
	mainWnd, _, _ = pCreateWindowExW.Call(0, uintptr(unsafe.Pointer(cls)), uintptr(unsafe.Pointer(ws("ERP Gratidão v1.2.12 • Sistema de Gestão Comercial"))), WS_OVERLAPPEDWINDOW|WS_VISIBLE, 20, 15, 1320, 790, 0, 0, hinst, 0)
	buildNavigation()
	showBoot()
