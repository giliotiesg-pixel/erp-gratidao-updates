...
func replaceSaleItems12115(saleID int64, items []SaleEditItem12115) error {
 if saleID<=0||len(items)==0{return fmt.Errorf("informe os itens da venda")}
 if ensureVendas12115Schema()!=nil{return fmt.Errorf("falha ao preparar vendas")}
 if scalar(fmt.Sprintf("SELECT status FROM sales WHERE id=%d",saleID))==""{return fmt.Errorf("venda nao encontrada")}
 if err:=execSQL("BEGIN IMMEDIATE");err!=nil{return err}; ok:=false; defer func(){if !ok{_=execSQL("ROLLBACK")}}()
 _=execSQL(fmt.Sprintf("DELETE FROM sale_items WHERE sale_id=%d",saleID))
 for _,it:=range items{if it.ProductID<=0||it.Qty<=0||it.UnitPrice<0{return fmt.Errorf("item invalido")}; rows,e:=queryRows(fmt.Sprintf("SELECT description,cost FROM products WHERE id=%d",it.ProductID),2);if e!=nil||len(rows)==0{return fmt.Errorf("produto nao encontrado")}; cost:=parseF(rows[0][1]); lt:=it.Qty*it.UnitPrice; lc:=it.Qty*cost; if e=execSQL(fmt.Sprintf("INSERT INTO sale_items(sale_id,product_id,product_description_snapshot,qty,unit_price,unit_cost,line_total,line_cost,line_profit) VALUES(%d,%d,'%s',%.4f,%.4f,%.4f,%.4f,%.4f,%.4f)",saleID,it.ProductID,esc(rows[0][0]),it.Qty,it.UnitPrice,cost,lt,lc,lt-lc));e!=nil{return e}}
 if e:=recalcSale12115(saleID);e!=nil{return e}; _=execSQL(fmt.Sprintf("INSERT INTO sale_changes_12115(sale_id,change_type,details) VALUES(%d,'ITENS','Itens/produtos/quantidades/valores alterados diretamente em Vendas')",saleID)); if e:=execSQL("COMMIT");e!=nil{return e};ok=true;return nil
}

func updateSaleInfo12115(saleID int64, customer,payment,date string) error {
 if saleID<=0{return fmt.Errorf("venda invalida")}; sets:=[]string{}
